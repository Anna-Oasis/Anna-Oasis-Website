...
// Fetch student details pending verification
export async function fetchStudentDetailsForVerification() {
  try {
    ...
    if (!res.data?.success)
      throw new Error(res.data?.message || "Failed to fetch manager details");
...
export async function updateStudentProfileApproval(
  approve: boolean,
  rollNo: string,
  comment: string,
) {
  try {
    ...
    if (!res.data?.success)
      throw new Error(res.data?.message || "Failed to update profile approval");
    return res.data;
  } catch (err: any) {
